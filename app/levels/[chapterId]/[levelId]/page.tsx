...
export async function generateMetadata({ params }: LevelPageProps): Promise<Metadata> {
  ...
  const levelContent = getLevelContent(levelId, "en");
  ...
  const description = meta.seo_description ?? `VibeCamp level ${levelId}: ${meta.title}. Estimated time: ${meta.estimated_minutes} minutes.`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        en: canonicalUrl,
        "zh-CN": `${siteUrl}/zh/levels/${chapterId}/${levelId}`,
      },
    },
    openGraph: {
...
export default async function LevelPage({ params }: LevelPageProps) {
  ...
  // Steps data
  const steps = getStepsByLevelId(levelId, "en");
  ...
  const siblingContents = siblingLevels
    .map((s) => {
      const c = getLevelContent(s.levelId, "en");
  ...
  const nextChapterContent = nextChapterFirst ? getLevelContent(nextChapterFirst.levelId, "en") : null;
  const canonicalUrl = `${siteUrl}/levels/${chapterId}/${levelId}`;
        ...
        <div className="max-w-5xl mx-auto flex h-14 items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            {isLoggedIn ? (
              ...
              <Link
                href={`/login?next=/levels/${chapterId}/${levelId}`}
        ...
        <div className="max-w-3xl mx-auto px-6 py-10">
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-400">
              <span className="flex items-center gap-1.5">
                ...
                {levelContent.meta.estimated_minutes} min
              </span>
              {isCompleted && (
          ...
          {isSkipped && (
            <div className="mb-6 px-5 py-4 rounded-2xl bg-amber-50 border border-amber-100 flex items-center gap-3">
              ...
              <span className="text-sm text-amber-700 font-medium">Skipped. You can come back and submit verification later.</span>
            </div>
          )}
          ...
          {!isLoggedIn && (
            <div className="mt-10 px-8 py-8 rounded-2xl border border-gray-100 bg-gray-50 text-center">
              ...
              <h3 className="text-lg font-black text-gray-900 mb-2">Log in to continue</h3>
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <Link
                  href={`/register?next=/levels/${chapterId}/${levelId}`}
              ...
              {nextChapterContent && nextChapterFirst && (
                <div className="mt-2">
                  ...
                  >
                    ...
                    <span className="text-indigo-600 font-medium">Next chapter: {nextChapterContent.meta.title}</span>
                  </Link>
                </div>
              )}
            </div>
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 flex items-center justify-between">
            ...
            {(isCompleted || isSkipped) && nextLevel ? (
              <Link
                href={`/levels/${nextLevel.chapterId}/${nextLevel.levelId}`}
